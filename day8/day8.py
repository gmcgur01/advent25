...
def main():
    ...
    limit = 10 if sys.argv[1] == "example.in" else 1000
    circuits = {}
    ...
    final_circuits = list({id(s): s for s in circuits.values()}.values())

    final_circuits.sort(key=lambda circuit: len(circuit), reverse=True)
    
    product = 1
    for circuit in final_circuits[:3]:
        product *= len(circuit)
